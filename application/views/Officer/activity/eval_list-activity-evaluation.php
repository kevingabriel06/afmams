				...
				<div class="table-responsive scrollbar">
					<table class="table table-hover table-striped overflow-hidden">
						...
						<tbody class="list" id="table-evaluation-body">
							...
							<?php endforeach; ?>
							<!-- NO ACTIVITY fallback row -->
							<tr id="no-activity-row" style="display:none;">
								<td colspan="6" class="text-center text-muted py-4">
									<span class="fas fa-calendar-times fa-2x"></span>
									<div class="fw-bold fs-6 mt-2">No Activity Found</div>
								</td>
							</tr>
						</tbody>
					</table>
			...
			<div class="modal-body">
				...
				<div class="row mb-3">
					<div class="col-md-6">
						...
						<input type="text" id="semester-filter" class="form-control" value="<?= htmlspecialchars($active_semester) ?>" readonly>
					</div>

					<div class="col-md-6">
						<label for="academic-year-filter" class="form-label">Academic Year</label>
						<input type="text" id="academic-year-filter" class="form-control" value="<?= htmlspecialchars($active_academic_year) ?>" readonly>
					</div>
				</div>
...
<script>
	window.applyFilters = function() {
		...
		activities.forEach(activity => {
			...
			if (selectedStatus === "" || activityStatus === selectedStatus) {
				activity.style.display = 'table-row'; // show row
				hasVisibleActivity = true;
			} else {
				activity.style.display = 'none'; // hide row
			}
		});
	...
	function toggleNoActivityMessage(showAny) {
		const message = document.getElementById('evaluation-table-fallback'); // your existing fallback div
		if (message) {
			message.style.display = showAny ? 'none' : 'block';
		}
	}
</script>
