										...
										<?php if ($item->student_id == $this->session->userdata('student_id')): ?>
											<div class="dropdown font-sans-serif btn-reveal-trigger">
												...
												<div class="dropdown-menu dropdown-menu-end py-3" aria-labelledby="post-album-action">
													<a class="dropdown-item text-danger" id="delete-post" data-post-id="<?php echo $item->post_id; ?>" data-url="<?php echo site_url('officer/community/delete-post'); ?>">Delete</a>
												</div>
											</div>
										<?php endif; ?>
									</div>
								...
								$(document).on('click', '.unshare-activity', function(e) {
									...
									Swal.fire({
										...
										if (result.isConfirmed) {
											$.ajax({
												url: "<?php echo site_url('officer/unshare-activity'); ?>",
												type: "POST",
												data: {
									...
									<div class="col">
										<div class="d-flex">
											...
											<div class="flex-1 position-relative ps-3">
												...
												<h6 class="fs-9 mb-0">
													<a href="<?php echo site_url('officer/activity-details/' . $item->activity_id); ?>">
				...
				?>
					<div class="d-flex btn-reveal-trigger mb-3">
						...
						<div class="flex-1 position-relative">
							<h6 class="fs-9 mb-1">
								<a href="<?php echo site_url('officer/activity-details/' . $activity->activity_id); ?>">
...
<script>
	$(document).ready(function() {
		...
		$('[id^=btn-like]').click(function() {
			...
			$.ajax({
				url: '<?= site_url("officer/community/like-post/") ?>' + postId, // Send to controller's like_post method
				type: 'POST',
				success: function(response) {
	...
	function updateLikeList(postId) {
		$.ajax({
			url: '<?= site_url('officer/view_likes/'); ?>' + postId,
			method: 'GET',
			success: function(response) {
...
<script>
	...
	$(document).on('submit', 'form[id^="commentForm"]', function(e) {
		...
		$.ajax({
			url: '<?= site_url("officer/community/add-comment"); ?>',
			type: 'POST',
			data: formData,
			dataType: 'json',
			success: function(response) {
		...
		$('#postForm').on('submit', function(e) {
			...
			Swal.fire({
				...
				if (result.isConfirmed) {
					...
					$.ajax({
						url: '<?php echo site_url("officer/community/add-post"); ?>',
						type: 'POST',
						data: formData,
						contentType: false,
						processData: false,
		...
		shareButton.addEventListener('click', function() {
			...
			Swal.fire({
				...
				if (result.isConfirmed) {
					...
					fetch("<?= site_url('officer/community/share-activity') ?>", {
						method: "POST",
						headers: {
