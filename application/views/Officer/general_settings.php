		...
		<div class="row g-4">
				...
				document.getElementById('generateAllQrBtn').addEventListener('click', function() {
					...
					Swal.fire({
						...
						if (result.isConfirmed) {
							...
							fetch('<?php echo site_url("officer/generate_bulk_qr"); ?>', {
									method: 'POST'
								})
								.then(res => res.json())
								.then(data => {
			...
			<div class="col-md-6">
				<div class="border rounded p-3 h-100 d-flex justify-content-between align-items-start">
					...
					<a href="<?= base_url('officer/record-cash-payment') ?>" class="btn btn-sm btn-outline-danger">
						Open
					</a>
		...
		$('#importForm').submit(function(e) {
			...
			Swal.fire({
				...
				if (result.isConfirmed) {
					...
					$.ajax({
						url: "<?= site_url('officer/import-students'); ?>",
						type: "POST",
						data: formData,
						contentType: false,
						processData: false,
