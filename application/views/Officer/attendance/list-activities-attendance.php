<div class="row gx-3">
								...
								<tr class="activity-row" data-semester="<?php echo $semester; ?>" data-academic-year="<?php echo $academicYear; ?>" data-start-date="<?php echo $activity->start_date; ?>" data-status="<?php echo $activity->status; ?>">
									<td class="align-middle text-nowrap px-6 py-2 activity">
										<div class="d-flex align-items-center">
											...
											<?php else: ?>
												<a class="ms-3 text-decoration-none text-dark fw-semibold d-block link-hover"
													href="<?php echo site_url('officer/list-attendees/' . $activity->activity_id); ?>">
													<?php echo $activity->activity_title; ?>
												</a>
											<?php endif; ?>
										</div>
	...
	<script>
		window.applyFilters = function() {
			...
			rows.forEach(row => {
				const activityStatus = row.getAttribute("data-status");

				if (selectedStatus === "" || selectedStatus === activityStatus) {
					row.style.display = "";
			...
			// Toggle only one fallback (the table row inside <tbody>)
			const noActivityRow = document.getElementById('no-activity-row');
			if (noActivityRow) {
				noActivityRow.style.display = hasVisibleActivity ? 'none' : 'table-row';
			}

			// Ensure fallback DIV is hidden always (or delete it from DOM if unused)
			const fallbackDiv = document.getElementById('attendance-table-fallback');
			if (fallbackDiv) {
