        ...
        <h5 class="mb-0">
            <?php foreach ($department as $dept): ?>
                <?php if ($dept_id == $dept->dept_id): ?>
                    List of Officers - <?php echo $dept->dept_name; ?>
                <?php endif; ?>
            <?php endforeach; ?>
        </h5>
    </div>
...
<div class="row gx-3">
                    ...
                    <table class="table table-hover table-striped overflow-hidden">
                        ...
                        <tbody class="list" id="table-ticket-body">
                            <?php foreach ($officers as $officer) : ?>
                                <?php if ($officer->dept_id == $dept_id): ?>
                                    <tr class="users-row" data-is-admin="<?php echo $officer->is_admin; ?>">
                                        ...
                                        <td class="status px-5 py-2">
                                            <?php if ($officer->is_admin === 'Yes'): ?>
                                                <span class="badge badge rounded-pill d-block p-2 badge-subtle-success">Department Admin
                                                    <span class="ms-1 fas fa-check" data-fa-transform="shrink-2"></span>
                                                </span>
                                            <?php elseif ($officer->is_admin === 'No'): ?>
                                                <span class="badge badge rounded-pill d-block p-2 badge-subtle-danger">Department Officer
                                            ...
                                            <div class="dropdown font-sans-serif btn-reveal-trigger">
                                                ...
                                                <div class="dropdown-menu dropdown-menu-end py-3"
                                                    ...
                                                    <?php if ($officer->is_admin === 'No'): ?>
                                                        <a class="dropdown-item text-success make-an-admin"
                                                            href="#"
                                                    ...
                                                    <?php elseif ($officer->is_admin === 'Yes'): ?>
                                                        <a class="dropdown-item text-warning remove-an-admin"
                                                            href="#"
                                                            data-student-id="<?php echo $officer->student_id; ?>"
                                                            data-status="No">
                ...
                <div class="modal-body">
                    <form>
                        ...
                        <select id="is_admin" class="form-select">
                            ...
                            <option value="Yes">Department Admin</option>
                            <option value="No">Department Officer</option>
                        </select>
                    </form>
                </div>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            document.querySelectorAll(".make-an-admin, .remove-an-admin").forEach((btn) => {
                btn.addEventListener("click", function(event) {
                    ...
                    alertify.confirm("Confirm Action", confirmationMessage,
                        function() {
                            fetch("<?php echo base_url('admin/manage-officers-department/update_status'); ?>", {
                                    method: "POST",
                                    headers: {
