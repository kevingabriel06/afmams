...
<div class="container">
	...
	<script>
					...
					for (const detectedFace of detectedFaces) {
						if (!detectedFacesSet.has(detectedFace)) {
							...
							if (student) {
								...
								const result = await fetch("<?= base_url('admin/attendance/detect_timein'); ?>", {
									method: 'POST',
									headers: {
										'Content-Type': 'application/json'
									},
		...
		scanner.addListener('scan', function(content) {
			...
			$.ajax({
				url: "<?php echo site_url('admin/attendance/detect_timein'); ?>",
				method: "POST",
				dataType: "json",
				data: {
					student_id: text,
				...
				success: function(response) {
					if (response.status === 'success') {
						Swal.fire({
							...
							imageUrl: response.profile_pic, // this should be part of your AJAX response
							imageAlt: 'Profile Picture',
							imageWidth: 300,
							imageHeight: 250,
							timer: 3000,
