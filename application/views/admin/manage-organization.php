<form id="privilegeForm">
    <input type="hidden" name="dept_id" value="<?= $org_id; ?>">
            ...
            <h5 class="mb-0">
                <?php foreach ($organization as $org): ?>
                    <?php if ($org_id == $org->org_id): ?>
                        List of Officers - <?= htmlspecialchars($org->org_name); ?>
                    <?php endif; ?>
                <?php endforeach; ?>
            </h5>
        </div>
                ...
                <table class="table table-hover table-striped mb-0">
                    ...
                    <tbody>
                        <?php foreach ($privileges as $privilege): ?>
                            <?php if ($privilege->org_id == $org_id): ?> <!-- Check if dept_id matches -->
                                <tr>
                                        ...
                                        $(document).on('click', '.delete-officer', function(e) {
                                            ...
                                            Swal.fire({
                                                ...
                                                if (result.isConfirmed) {
                                                    $.ajax({
                                                        url: "<?php echo site_url('admin/manage-officers-organization/delete-officer'); ?>",
                                                        type: 'POST',
                                                        data: {
                                                            id: officerId
                                                        },
    ...
    $(document).ready(function() {
        $('#privilegeForm').on('submit', function(event) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "<?= site_url('admin/manage-officers-organization/update_privileges') ?>",
                        method: "POST",
