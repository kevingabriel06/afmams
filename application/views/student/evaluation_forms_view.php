...
<div class="row g-0">
    <form id="answerForm" class="needs-validation" novalidate="" data-dropzone="data-dropzone" enctype="multipart/form-data">
        ...
        <div class="card mt-3 shadow-sm">
            <div class="card-body bg-body-tertiary">
                <div class="row justify-content-between align-items-center">
                    ...
                    <div class="col-auto">
                        <button class="btn btn-danger btn-sm me-2" type="button" onlick="answerForm.reset()">
                            Cancel
                        </button>
                        <button class="btn btn-primary btn-sm me-2" type="submit" data-form-id="<?php echo $forms['form_id']; ?>">
                            Submit
                        </button>
                    </div>
                </div>
        ...
        if (type === "short") {
            newField = `
                <div class="card shadow-sm mb-3" id="${fieldId}">
                    <div class="card-body">
                        <input type="hidden" name="id[]" value="${form_fields_id}"/>
...
<script>
    $("#answerForm").on("submit", function(e) {
        ...
        // Collect answers, field IDs, and types
        $("#form-fields .card").each(function() {
            const answerInput = $(this).find("[name='answers[]']");
            const typeInput = $(this).find("[name='type[]']");
            const idInput = $(this).find("input[name='id[]']");

            if (answerInput.length && idInput.length && typeInput.length) {
                formData.append("answers[]", answerInput.val());
